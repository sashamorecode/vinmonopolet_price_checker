...
def worker(*splitLinks):
    for link in splitLinks:
        ...
        print(drink)
        append_list_as_row("data1.csv",drink)
...
if __name__ == '__main__':
    jobs = []
    for i in range(0,9):
        length = math.floor(len(links)/10)
        splitLinks = links[i*length:(i+1)*length]
        p = multiprocessing.Process(target=worker, args=(splitLinks))
        jobs.append(p)
        p.start()
